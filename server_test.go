...
func init() {
	// sbitRPC, found := os.LookupEnv("SBIT_RPC")
	// if !found {
	// 	fmt.Println("Please specify RPC url with SBIT_RPC environment variable")
	// 	os.Exit(1)
	// }

	// sbitRPCURL, err := url.Parse(sbitRPC)
	sbitRPCURL, err := url.Parse("http://user:pass@localhost:22302")
	if err != nil {
		log.Println("Invalid SBIT_RPC", sbitRPCURL.String())
	}

	testRPCURL = sbitRPCURL
}
