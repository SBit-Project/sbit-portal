...
1. Interact with Smart Contract in the browser using `sbitjs`.
...
> For details of how RPC authorization works, see [SBIT Portal Authorization Design](https://github.com/SBit-Project/sbit-portal/blob/master/DESIGN.md). `sbitjs` handles authorization transparently, so you don't need to think about this in practice.

# Developing The DApp UI

A DApp is essentially a UI wrapper for accessing a set of smart contracts' methods.

In [solar.development.json](https://github.com/hayeah/sbit-dapp-counter/blob/master/solar.development.json.example) there is information about the deployed contracts, which sbitjs can use to `call` or `send` to a contract.
...
These two constants are defined in [config/development.js](https://github.com/hayeah/sbit-dapp-counter/blob/master/config/development.js).
...
See [api.ts](https://github.com/hayeah/sbit-dapp-counter/blob/master/src/api.ts) for an example of building a simple API wrapper.
